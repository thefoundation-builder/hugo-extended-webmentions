...
RUN  ( test -e /etc/webmention-static-ci || git clone --recurse-submodules https://gitlab.com/the-foundation/webmention-static-ci.git /etc/webmention-static-ci) && \
...
RUN head -c 12 /dev/urandom >/etc/.tmp && test -e  /etc/webmention-static-ci/ && (cd  /etc/webmention-static-ci/ ;git pull ) || true 
